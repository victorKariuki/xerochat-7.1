...
class Subscriber_manager extends Home
{
    ...
    { 
      $page_dropdown = $this->get_facebook_instagram_dropdown($this->session->userdata("facebook_rx_fb_user_info"));
      $page_dropdown2 = $this->get_facebook_instagram_dropdown($this->session->userdata("facebook_rx_fb_user_info"),"selected_page_id","selected_page_id","width: 100%;");
      $data['page_dropdown'] = $page_dropdown;
      $data['page_dropdown2'] = $page_dropdown2;
    ...
    { 
      ...
      $display_columns = array("#","id","group_name","label_id","page_name","actions","social_media");
      ...
      for($i=0;$i<count($info);$i++) 
      {
        ...
        if($info[$i]['social_media']=='fb') $info[$i]['page_name'] = "<a title='".$this->lang->line("Visit Page")."' target='_BLANK' href='https://facebook.com/".$info[$i]['pageid']."'>".$info[$i]['page_name']."</a>";
       else $info[$i]['page_name'] = "<a title='".$this->lang->line("Visit Account")."' target='_BLANK' href='https://instagram.com/".$info[$i]['insta_username']."'>".$info[$i]['insta_username']."</a>";
        ...
        if($info[$i]['unsubscribe'] == '1')
          ...
          $info[$i]['actions'] = "<a href='#' class='btn btn-outline-danger btn-circle delete_label' social_media='".$info[$i]['social_media']."'  table_id='".$info[$i]['id']."' title='".$this->lang->line("Delete Label")."'><i class='fas fa-trash-alt'></i></a>";

        $info[$i]['social_media'] = $info[$i]['social_media']=='fb' ? 'Facebook' : 'Instagram';
      }
    ...
    public function sync_subscribers($switch_to_instagram='0')
    {
        ...
        $data['switch_to_instagram'] = $switch_to_instagram;
    ...
    public function get_page_details()
    {
        ...
        $switch_to_instagram = $this->input->post('switch_to_instagram',true);
        $facebook_rx_fb_user_info_id  =  $this->session->userdata('facebook_rx_fb_user_info');
        $this->session->set_userdata('sync_subscribers_get_page_details_page_table_id',$page_table_id);
        $social_media = $switch_to_instagram=='1' ? 'ig' : 'fb';
        ...
        else  if(!empty($page_info[0]['insta_current_unsubscribed_lead_count'])) $unsubscribed = custom_number_format($page_info[0]['insta_current_unsubscribed_lead_count']);

        $conversation_subscriber_info = $this->basic->get_data('messenger_bot_subscriber',array('where'=>array('user_id'=>$this->user_id,'page_table_id'=>$page_table_id,'client_thread_id !='=>'','social_media'=>$social_media)),array('count(id) as total_subscriber'));
        $conversation_unavailable_info = $this->basic->get_data('messenger_bot_subscriber',array('where'=>array('user_id'=>$this->user_id,'page_table_id'=>$page_table_id,'client_thread_id !='=>'','unavailable_conversation'=>'1','social_media'=>$social_media)),array('count(id) as unavailable_conversation'));
        ...
        $middle_column_content = '
            <div class="card main_card">
                <div class="card-header">
                  <h4 class="full_width">
                    '.$title.'                    
                      <label class="custom-switch float-right d-none">
                        <input type="checkbox" name="switch_to_instagram" id="switch_to_instagram" value="1" class="custom-switch-input" '.$checked.'>
                        <span class="custom-switch-indicator"></span>
                        <span class="custom-switch-description">'.$insta_user.'</span>
                      </label>
                  </h4>
                </div>
                <div class="card-body">
                  ...
                  <div class="row margin-top-50">
                    ...
                    <div class="'.$col.'">
                      <div class="product-item pb-3">
                        ...
                        <div class="product-details">
                          <div class="product-name">'.$this->lang->line("Live Chat").'</div>                      
                          <div class="product-cta">
                            '.$details.'
                          </div>
                        </div>
    ...
    { 
        ...
        $switch_to_instagram = $this->input->post("switch_to_instagram");
        $social_media = $switch_to_instagram=='1'?'ig':'fb';
        ...
        $where_custom="user_id = ".$this->user_id." AND social_media='".$social_media."' AND page_table_id = ".$page_id;
    ...
    public function import_lead_action(){
        ...
        $sql = "SELECT count(id) as permission_count FROM `messenger_bot_subscriber` WHERE page_table_id='$facebook_rx_fb_page_info_id' AND social_media='".$social_media."' AND permission='1' AND user_id=".$this->user_id;
        $count_data = $this->db->query($sql)->row_array();

        $sql2 = "SELECT count(id) as permission_count FROM `messenger_bot_subscriber` WHERE page_table_id='$facebook_rx_fb_page_info_id' AND social_media='".$social_media."' AND permission='0' AND user_id=".$this->user_id;
    ...
    {        
       ...
       if(function_exists('ini_set')){
        ...
        $switch_to_instagram = $user_id_and_page_id[2];
        $social_media = $switch_to_instagram=='1' ? 'ig' : 'fb';

        $where['where'] = array('user_id' => $this->user_id,'page_table_id' => $page_id,'social_media'=>$social_media); 
    ...
    { 
        ...
        $table="messenger_bot_subscriber";
        $select = "messenger_bot_subscriber.*,page_name,insta_username";
        $this->db->where($where_custom);
        $info=$this->basic->get_data($table,$where='',$select,$join,$limit,$start,$order_by,$group_by='');
    ...
    public function bulk_group_assign()
    {
        ...
        foreach ($subscriber_data as $key => $value) 
        {
         ...
         $sql = "SELECT count(id) as permission_count FROM `messenger_bot_subscriber` WHERE social_media='$social_media' AND page_table_id='$page_id' AND permission='1' AND user_id=".$this->user_id;
         $count_data = $this->db->query($sql)->row_array();

         $sql2 = "SELECT count(id) as permission_count FROM `messenger_bot_subscriber` WHERE  social_media='$social_media' AND page_table_id='$page_id' AND permission='0' AND user_id=".$this->user_id;
         $count_data2 = $this->db->query($sql2)->row_array();
    ...
    public function subscriber_actions_modal()
    {
      ...
      if($this->addon_exist("custom_field_manager") && $social_media=='fb')
      {
        if($this->session->userdata('user_type') != 'Admin' && !in_array(292,$this->module_access))
        {
        ...
        <div class="'.$colmid_class.' colmid" id="middle_column">
            <div class="card main_card">
              ...
              <div class="card-footer">
                <a class="btn btn-primary float-left" href="" data-social-media="'.$subscriber_data['social_media'].'" data-subscribe-id="'.$subscriber_data['subscribe_id'].'" data-id="'.$subscriber_data['id'].'" data-page-id="'.$subscriber_data['page_table_id'].'" id="save_changes"><i class="fas fa-save"></i> '.$this->lang->line("Save Changes").'</a>
    ...
    public function create_label_and_assign()
    {
      ...
      else
      {
        ...
        //   $insert_data['label_id'] = isset($response['id']) ? $response['id'] : "";
        // }
